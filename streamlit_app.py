...
def main():
    ...
    if uploaded_file is not None:
        ...
        with st.spinner('Классификация изображения...'):
            # Загружаем и отображаем изображение DICOM
            dicom = pydicom.dcmread(filepath)
            image_array = dicom.pixel_array

            # Уменьшаем размер изображения вдвое
            resized_image = cv2.resize(image_array, (int(image_array.shape[1] / 2), int(image_array.shape[0] / 2)))

            # Нормализуем значения пикселей в диапазон [0, 255]
            ...
            result_text = classify_dicom(filepath)
            st.write("### Заключение  классификатора:")
            # Форматируем вывод в цветной блок
            if "здоровое" in result_text.lower() or "1 (или 2)" in result_text:
                st.markdown('<div style="background-color: #008000; padding: 10px; border-radius: 5px;">'
                            '<p style="color: white;">{}</p></div>'.format(result_text), unsafe_allow_html=True)
            elif "3 (или 4)" in result_text:
                st.markdown('<div style="background-color: #FF33FF; padding: 10px; border-radius: 5px;">'
                            '<p style="color: black;">{}</p></div>'.format(result_text), unsafe_allow_html=True)
            else:
                st.markdown('<div style="background-color: #FF5733; padding: 10px; border-radius: 5px;">'
                            '<p style="color: white;">{}</p></div>'.format(result_text), unsafe_allow_html=True)
        # По завершении классификации спиннер исчезает
